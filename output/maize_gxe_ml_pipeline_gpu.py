...
def log_gpu_mem(tag: str) -> None:
    m = check_gpu_mem_mib()
    if m is not None:
        logging.info("[GPU mem MiB] %s: %.1f", tag, m)
...
def setup_logging(output_dir: Path, log_filename: str = "pipeline_gpu.log") -> None:
...
def step1_lock_analytical_unit(
    ...
    out_path = output_dir / f"pheno_env{artifact_suffix}.csv"
...
def step2_build_genotype_matrix(
    ...
    X = X.fillna(0.0)
    geno[snp_names] = X
    out = output_dir / f"genotypes_{cohort.lower()}{artifact_suffix}.csv"
...
def step3_validate_joins(
    ...
    full_path = output_dir / f"full_dataset{artifact_suffix}.csv"
    full.to_csv(full_path, index=False)
    logging.info("Saved full_dataset%s.csv (%s rows)", artifact_suffix, len(full))
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=_repo / "pipeline_config.yaml",
        help="YAML config (expects use_gpu, cuda_device; paths relative to repo)",
    )
    ...
    def mem_sample() -> None:
        nonlocal peak_mem_mib
        m = check_gpu_mem_mib()
        if m is not None:
            peak_mem_mib = m if peak_mem_mib is None else max(peak_mem_mib, m)
    ...
    joblib.dump(env_scaler, out / f"env_scaler{SUF}.pkl")
    ...
    joblib.dump(final_pca_pipe, out / f"pca_pipeline{SUF}.pkl")
    ...
    joblib.dump(ridge, out / f"ridge_snp_baseline{SUF}.pkl")
    ...
    pd.Series(nongeno_feature_cols, name="env_and_gxe_column").to_csv(
        out / f"env_gxe_feature_columns{SUF}.csv", index=False
    )
    ...
    n_splits_cv = int(hypers.get("group_cv_splits", 5))
    rs_pipe = int(cfg.get("random_state", 42))

    time_search = time_oof = time_phase_b = float("nan")
    ...
    time_oof = time.perf_counter() - t0
    log_gpu_mem("post OOF loop")
    mem_sample()
    ...
    by_env.to_csv(out / f"metrics_by_env_oof{SUF}.csv", index=False)
    ...
    rankings.to_csv(out / f"rankings_topk_per_env{SUF}.csv", index=False)
    ...
    fi.to_csv(out / f"perm_importance{SUF}.csv", index=False)
    ...
    bucketed.to_csv(out / f"perm_importance_buckets{SUF}.csv", index=False)
    ...
    if cfg.get("use_shap", True):
        try:
            ...
            plt.tight_layout()
            plt.savefig(out / f"shap{SUF}.png", dpi=150, bbox_inches="tight")
            plt.close()
    ...
    summary = {
        ...
        "artifact_suffix": SUF,
        "phase_b_model_path": str(phase_b_path),
    }
    ...
    with open(out / f"run_summary{SUF}.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, default=_json_default)

    peak_gb = (peak_mem_mib / 1024.0) if peak_mem_mib is not None else None
    gpu_summary = {
        ...
        "time_seconds": {
            "randomized_search": time_search,
            "oof_loop": time_oof,
            "phase_b_fit": time_phase_b,
            "micro_bench_gpu": time_gpu_bench,
            "micro_bench_cpu_xgb": time_cpu_bench,
        },
        "peak_mem_gb_nvidia_smi": peak_gb,
        "oof_global_metrics": glob_metrics,
    }
    with open(out / "gpu_run_summary.json", "w", encoding="utf-8") as f:
        json.dump(gpu_summary, f, indent=2, default=_json_default)
